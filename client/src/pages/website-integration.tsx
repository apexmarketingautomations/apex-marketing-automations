...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { color: string; label: string }> = {
    ...
    trained: { color: "bg-emerald-500/20 text-emerald-400 border-emerald-500/30", label: "AI Trained" },
...
export default function WebsiteIntegration() {
  ...
  const addMutation = useMutation({
    ...
    onSuccess: (site) => {
      ...
      setActiveTab("manage");
      toast({ title: "Website connected", description: `${site.name} has been added to your account.` });
    },
    onError: () => toast({ title: "Failed to connect website", variant: "destructive" }),
  });
  ...
  const scrapeMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast({ title: "AI Training Started", description: "Your chatbot is being trained on the website content. This takes about 10 seconds." });
      const pollInterval = setInterval(async () => {
        try {
          ...
          if (job.state === "completed") {
            ...
            queryClient.invalidateQueries({ queryKey: ["/api/client-websites"] });
            toast({ title: "Training Complete", description: "Your AI chatbot is ready! You can now get the embed code." });
          }
        } catch {
          clearInterval(pollInterval);
        }
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="bg-slate-800/50 border border-slate-700/50">
          <TabsTrigger value="sites" className="data-[state=active]:bg-cyan-600 data-[state=active]:text-white">
            <Globe className="w-4 h-4 mr-2" /> Connected Sites
          </TabsTrigger>
          ...
          {isLoading ? (
            ...
            <Card className="bg-slate-800/40 border-slate-700/50">
              <CardContent className="py-16 text-center">
                ...
                <h3 className="text-lg font-semibold text-white mb-2">No Websites Connected</h3>
                <p className="text-slate-400 mb-6 max-w-md mx-auto">
                  Connect a client's existing website to train an AI chatbot, generate an embeddable widget, and manage everything from here.
                </p>
                <Button onClick={() => setShowAddDialog(true)} className="bg-cyan-600 hover:bg-cyan-500" data-testid="button-add-first-website">
                  <Plus className="w-4 h-4 mr-2" /> Connect Your First Website
                </Button>
              </CardContent>
            </Card>
          ) : (
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          {site.pagesCrawled && site.pagesCrawled > 0 && (
                            <Badge variant="outline" className="bg-slate-700/30 text-slate-300 border-slate-600 text-xs">
              ...
              <div className="lg:col-span-2 space-y-6">
                <Card className="bg-slate-800/40 border-slate-700/50">
                  <CardHeader>
                    ...
                    <CardDescription>
                      Scrape the website and train an AI chatbot that can answer questions about the business.
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    ...
                    {selectedSite.status === "trained" && (
                      <div className="flex items-center gap-2 p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                        <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                        <span className="text-sm text-emerald-300">
                          AI chatbot trained on {selectedSite.pagesCrawled} pages. Ready for embedding.
                        </span>
                      </div>
                    )}
                    ...
                    {selectedSite.status === "training" && (
                      <div className="flex items-center gap-2 p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
                        ...
                        <span className="text-sm text-amber-300">
                          Training in progress... This usually takes about 10 seconds.
                        </span>
                      </div>
                    )}
                ...
                <Card className="bg-slate-800/40 border-slate-700/50">
                  <CardHeader>
                    ...
                    <CardDescription>Customize how the chat widget looks on the client's website.</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                ...
                <Card className="bg-slate-800/40 border-slate-700/50">
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-white font-mono">{selectedSite.pagesCrawled || 0}</span>
                    </div>
                    <div className="flex justify-between text-sm">
            ...
            <div className="space-y-6">
              {selectedSite.status !== "trained" && (
                <div className="flex items-center gap-3 p-4 rounded-xl bg-amber-500/10 border border-amber-500/20">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-amber-400/70">Go to the Manage tab and train the AI chatbot before embedding the widget. The widget will work without training, but responses will be generic.</p>
                  </div>
                </div>
              )}
              ...
              <Card className="bg-slate-800/40 border-slate-700/50">
                <CardHeader>
                  ...
                  <CardDescription>
                    Copy this code and paste it just before the closing &lt;/body&gt; tag on your client's website.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {!embedCode ? (
                    ...
                    <>
                      ...
                      <div className="bg-slate-900/50 border border-slate-700/30 rounded-xl p-4 space-y-3">
                        ...
                        <ol className="text-sm text-slate-400 space-y-2 list-decimal list-inside">
                          ...
                          <li>Open your client's website HTML (or CMS editor)</li>
                          <li>Paste the code just before the closing <code className="text-cyan-400 bg-slate-800 px-1 rounded">&lt;/body&gt;</code> tag</li>
                          <li>Save and publish — the chat widget will appear immediately</li>
                        </ol>
                      </div>
                    </>
                  )}
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>Live preview of the client's website</CardDescription>
                  </div>
                  <Button
                    variant="outline"
                    size="sm"
        ...
        <DialogContent className="bg-slate-900 border-slate-700 text-white">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Globe className="w-5 h-5 text-cyan-400" /> Connect a Client Website
            </DialogTitle>
          </DialogHeader>
