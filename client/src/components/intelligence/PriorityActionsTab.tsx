...
function ScoreBar({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="flex items-center gap-1.5">
      <span className="text-[8px] text-slate-600 w-12 text-right">{label}</span>
      <div className="flex-1 h-1 bg-white/[0.04] rounded-full overflow-hidden">
        <div className={`h-full rounded-full ${color}`} style={{ width: `${value}%` }} />
      </div>
      <span className="text-[8px] text-slate-500 tabular-nums w-6">{value}</span>
    </div>
  );
}
    ...
    <div className="flex-1 flex flex-col overflow-hidden" data-testid="panel-priority-actions">
      <div className="px-3 py-2 flex items-center justify-between border-b border-white/[0.04]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Priority Actions</span>
          {(summary?.criticalCount ?? 0) > 0 && (
            <span className="px-1 py-px text-[8px] bg-red-500/20 text-red-400 rounded font-bold border border-red-500/20">
      ...
      <div className="px-3 py-1.5 flex items-center gap-1 border-b border-white/[0.04] overflow-x-auto scrollbar-none">
        ...
        {[null, "critical", "high", "medium", "low"].map((p) => (
          <button
            ...
            className={`text-[8px] px-1.5 py-0.5 rounded font-medium transition-colors whitespace-nowrap ${
              filterPriority === p
        ...
        {filtered.length === 0 ? (
          <div className="flex-1 flex items-center justify-center p-8 text-center">
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-slate-600">No pending actions</p>
              <p className="text-[9px] text-slate-700">Apex is monitoring your platform for opportunities and issues</p>
            </div>
          </div>
        ) : (
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 mb-0.5">
                          ...
                          <span className={`text-[8px] font-semibold uppercase ${pCfg.color}`}>{action.priority}</span>
                          <span className="text-[7px] text-slate-700">{action.category}</span>
                          <span className="ml-auto text-[7px] text-slate-700 font-mono">{action.compositeScore}pts</span>
                        </div>
                        <p className="text-[10px] text-white font-medium leading-tight">{action.title}</p>
                        {!isExpanded && (
                          <p className="text-[9px] text-slate-500 truncate mt-0.5">{action.description}</p>
                        )}
                        ...
                        {isExpanded && (
                          ...
                          >
                            <p className="text-[9px] text-slate-400 leading-relaxed">{action.description}</p>
                            {action.whyThisMatters && (
                              <p className="text-[8px] text-violet-400 italic">{action.whyThisMatters}</p>
                            )}
                            ...
                            {action.suggestedAction && (
                              <p className="text-[8px] text-amber-400">→ {action.suggestedAction}</p>
                            )}
                            <div className="flex items-center gap-1.5 pt-1">
                              {action.navigateTo && (
                                <button
                                  onClick={(e) => { e.stopPropagation(); setLocation(action.navigateTo!); }}
                                  className="flex items-center gap-1 text-[8px] px-2 py-1 rounded bg-violet-500/20 text-violet-400 border border-violet-500/20 hover:bg-violet-500/30 transition-colors font-medium"
                                  data-testid={`button-act-${action.id}`}
                                >
                                  Take Action <ChevronRight size={8} />
                                </button>
                              )}
                              <button
                                onClick={(e) => { e.stopPropagation(); snoozeMutation.mutate({ actionId: action.id, hours: 24 }); }}
                                className="flex items-center gap-1 text-[8px] px-2 py-1 rounded bg-white/5 text-slate-500 hover:text-slate-300 hover:bg-white/10 transition-colors"
                                data-testid={`button-snooze-${action.id}`}
                              >
                                <Clock size={8} /> Snooze 24h
                              </button>
                              <button
                                onClick={(e) => { e.stopPropagation(); dismissMutation.mutate(action.id); }}
                                className="flex items-center gap-1 text-[8px] px-2 py-1 rounded bg-white/5 text-slate-500 hover:text-red-400 hover:bg-red-500/10 transition-colors"
                                data-testid={`button-dismiss-${action.id}`}
                              >
                                <XCircle size={8} /> Dismiss
                              </button>
                            </div>
                          </motion.div>
                        )}
