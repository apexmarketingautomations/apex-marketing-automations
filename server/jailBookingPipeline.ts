 ...
 *
 * Data flow:
 *   Nimble agent (form submit → paginate → detail records)
 *     → BookingRecord[]
 *     → dedup by county+booking_id
 *     → legalSignals  (raw intake — caseIntelligence picks up every 30 min)
 *     → legalLeads    (immediately available for attorney delivery)
 *     → contacts      (for SMS/email outreach via existing pipeline)
 *
 * Requires: NIMBLE_API_KEY  (set in Railway env vars)
 * Schedule: every 4 hours, staggered 3 s between counties
 */
...
export interface CountyBookingConfig {
  ...
  bookingUrl: string;  // Primary booking search URL
...
async function runNimbleAgent(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.warn(`[JAIL-BOOKING] Nimble HTTP ${res.status} for ${agentName}: ${preview}`);
      return [];
    }
    ...
    return records.filter((r): r is BookingRecord => !!r && typeof r === "object" && !!r.full_name);
  } catch (err: any) {
    clearTimeout(timer);
    console.warn(`[JAIL-BOOKING] ${agentName} agent error: ${err?.message || "unknown"}`);
    return [];
  }
}
...
type SignalType     = "dui_arrest" | "arrest_record";
...
function classifyCharges(charges: string[]): ChargeClassification {
  ...
  if (/MURDER|HOMICIDE|MANSLAUGHTER|ATTEMPTED\s*MURDER/.test(text))
    return { signalType: "arrest_record", legalVertical: "criminal",         urgency: "critical", chargeCategory: "Violent Felony"   };
  if (/TRAFFICKING|KIDNAP|ARSON|ROBBERY\s*WITH/.test(text))
    return { signalType: "arrest_record", legalVertical: "criminal",         urgency: "critical", chargeCategory: "Serious Felony"   };
  if (/FELONY|BURGLARY|GRAND\s*THEFT|FRAUD|FORGERY/.test(text))
    return { signalType: "arrest_record", legalVertical: "criminal",         urgency: "high",     chargeCategory: "Felony"           };
  if (/AGGRAVATED|BATTERY|ASSAULT/.test(text))
    return { signalType: "arrest_record", legalVertical: "criminal",         urgency: "high",     chargeCategory: "Assault/Battery"  };
  if (/DOMESTIC|DOMESTIC\s*VIOLENCE|INJUNCTION|STALKING/.test(text))
    return { signalType: "arrest_record", legalVertical: "family",           urgency: "high",     chargeCategory: "Domestic"         };
  if (/HIT\s*AND\s*RUN|RECKLESS\s*DRIVING|FLEEING|LEAVING\s*SCENE/.test(text))
    return { signalType: "arrest_record", legalVertical: "traffic",          urgency: "high",     chargeCategory: "Reckless/Hit&Run" };
  if (/DRUG|COCAINE|HEROIN|FENTANYL|METH|MARIJUANA|CONTROLLED\s*SUBSTANCE/.test(text))
    return { signalType: "arrest_record", legalVertical: "criminal",         urgency: "medium",   chargeCategory: "Drug Offense"     };
  if (/TRAFFIC|SUSPENDED\s*LICENSE|NO\s*VALID/.test(text))
    return { signalType: "arrest_record", legalVertical: "traffic",          urgency: "medium",   chargeCategory: "Traffic Offense"  };
  if (/THEFT|SHOPLIFTING|PETTY/.test(text))
    return { signalType: "arrest_record", legalVertical: "criminal",         urgency: "low",      chargeCategory: "Petty/Misdemeanor"};

  return { signalType: "arrest_record",   legalVertical: "criminal",         urgency: "medium",   chargeCategory: "Other"            };
}
...
function buildFallbackHash(county: string, record: BookingRecord, charges: string[]): string {
  const key = [county, record.full_name, record.booking_date, charges[0] || ""].join("|");
  return crypto.createHash("sha256").update(key).digest("hex").slice(0, 24).toUpperCase();
}
...
async function persistBookingRecord(
  ...
  // Dedup: prefer booking_id, fall back to name+date+charge
  ...
  const [signal] = await db.insert(legalSignals).values({
    ...
    status:            "raw",
    rawData:           rawPayload,
    detectedAt:        record.booking_date
      ? new Date(record.booking_date)
      : new Date(),
  }).returning({ id: legalSignals.id });

  // ── 2. Insert to legalLeads (high/critical only — don't pollute with noise) ─
...
async function runCountyScrape(
  ...
  console.log(`[JAIL-BOOKING] ${config.county}: starting agent ${config.agentName} (${fromStr} → ${toStr})`);

  // Primary run: 72h window
  let records = await runNimbleAgent(config.agentName, {
    booking_date_from: fromStr,
    booking_date_to:   toStr,
  });

  // Fallback: expand to 7 days if nothing found
  if (records.length === 0) {
    const sevenDaysAgo = new Date(dateTo.getTime() - 7 * 24 * 60 * 60 * 1000);
    console.log(`[JAIL-BOOKING] ${config.county}: no results for 72h — retrying 7-day window`);
    records = await runNimbleAgent(config.agentName, {
  ...
  console.log(`[JAIL-BOOKING] ${config.county}: ${records.length} records returned by agent`);

  for (const record of records) {
    try {
      const result = await persistBookingRecord(record, config);
...
export async function runAllCountyBookings(options: RunOptions = {}): Promise<void> {
  ...
  const configs = COUNTY_BOOKING_CONFIGS.filter(c =>
    c.enabled && (!options.counties || options.counties.includes(c.county))
  );

  console.log(`[JAIL-BOOKING] Run starting — ${configs.length} counties, ${daysBack}d lookback`);

  const totals: ScrapeStats = { inserted: 0, skipped: 0, errors: 0, records: 0 };

  for (const config of configs) {
    try {
      const result = await runCountyScrape(config, dateFrom, dateTo);
      totals.inserted += result.inserted;
      totals.skipped  += result.skipped;
      totals.errors   += result.errors;
      totals.records  += result.records;
      console.log(`[JAIL-BOOKING] ${config.county}: +${result.inserted} new | ${result.skipped} dupes | ${result.errors} errors`);
    } catch (err: any) {
      totals.errors++;
      console.error(`[JAIL-BOOKING] ${config.county}: fatal error — ${err?.message}`);
    }
    // Stagger requests — be a good API citizen
    await new Promise(r => setTimeout(r, STAGGER_BETWEEN_MS));
  }

  lastRunAt = new Date();
  console.log(
    `[JAIL-BOOKING] Run complete — counties=${configs.length} records=${totals.records} ` +
    `inserted=${totals.inserted} dupes=${totals.skipped} errors=${totals.errors}`,
  );
}
...
export function startJailBookingScheduler(): void {
  ...
  // First run 2 minutes post-boot so DB and tables are fully ready
  setTimeout(() => {
  ...
  console.log("[JAIL-BOOKING] Scheduler started — first run in 2 min, then every 4h (11 counties)");
}
