...
export function registerAuthRoutes(app: Express) {
  ...
  app.use("/api", (req, res, next) => {
    ...
    const openPaths = ["/api/auth/", "/api/login", "/api/logout", "/api/callback", "/api/stripe/webhook", "/api/webhook/", "/api/webhooks/", "/api/snapshots/marketplace", "/api/v1/serve-native-ad", "/api/v1/ad-click/", "/api/crash-reports/health", "/api/crash-reports/test-harness"];
    ...
    if (fullPath === "/api/meta-test-send") return next();
    if (fullPath === "/api/meta-dm-catchup") return next();
    ...
    // Public adaptation directives are needed by the public card page itself
    // (no auth on the card means no auth on its render hints either).
    if (fullPath.startsWith("/api/intelligence/cards/by-slug/")) return next();
