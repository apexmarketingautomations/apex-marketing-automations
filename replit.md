...
- **CRM & Lead Management**: Provides multi-tenant accounts, a visual deal pipeline, multi-channel contact management, and a form builder. Public form intake at `POST /api/public/form/:token` resolves a sub-account by `webhook_token`, creates a Contact + Deal in the "New Lead" stage, and fires `new_lead`/`OnNewLead` automation triggers; new clients are onboarded by issuing a token and adding one row — no code changes needed. The route is whitelisted in `server/routes/auth.ts`, `server/csrfProtection.ts`, and `server/routes/public-platform.ts`. Lookup is hardened against duplicate-token rows by refusing to serve when `webhook_token` resolves to more than one sub-account.
- **Intelligence Layers**:
