...
- **Integrations Hub**: Credential-based service connections for 18 providers (Google Suite, Slack, Zapier, QuickBooks, Twilio, Stripe, HubSpot, Mailchimp, Facebook, ElevenLabs, WhatsApp Business), config stored in `integration_connections.config`. Now includes platform-managed OAuth for Google and Meta: OAuth initiation/callback endpoints, token storage in `oauth_tokens` table, automatic token refresh via `server/tokenService.ts`, provider asset listing (calendars, sheets, pages, ad accounts), integration event logging in `integration_events` table, asset selection persistence in `provider_assets` table, and health-check endpoint. Legacy credential-based connections coexist with new OAuth connections via `connectionType` field on `integration_connections`.
...
- **WhatsApp Business API**: Dedicated WhatsApp Business integration via Twilio. Features: template message management (CRUD via `whatsapp_templates` table), interactive messages (buttons, quick replies, list messages), delivery status tracking (sent/delivered/read), WhatsApp-specific branding in unified inbox, SendWhatsApp workflow action type, OnWhatsAppReply trigger. Status webhook at `/api/whatsapp-status`.
...
- **Twilio**: Phone number provisioning, SMS webhook with AI auto-reply, WhatsApp Business API messaging (templates, interactive messages, delivery tracking).
