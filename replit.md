...
- **Shared Intelligence Layer**: Cross-account org-wide learning system. `shared_insights` table stores generalized patterns (FAQs, objections, reply patterns, conversion signals, trends) extracted from DM conversations via `server/services/insightExtractor.ts`. Fire-and-forget extraction runs after each DM reply in `webhooks.ts`. Insights are injected into DM bot prompts via `fetchRelevantInsights()` in `dmContextAssembler.ts` (ORGANIZATIONAL LEARNINGS section) and operator context via `contextBuilder.ts` (Customer Intelligence section). PII/identity guards: extraction prompt forbids PII, `IDENTITY_REJECT` regex blocks account-identifying terms, `maskPiiForLogs()` strips PII post-extraction. Decay-weighted scoring: `confidence * occurrence_count * EXP(-decay_rate * age_days)`. Admin APIs: `GET /api/intelligence/insights` (category/limit filters), `POST /api/intelligence/extract` (batch backfill). Both admin-secret protected (`x-admin-secret` header), auth/CSRF bypassed.
