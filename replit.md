...
- **Sentinel Geofence Ingest** (`POST /api/v1/sentinel-ingest`): MAID-to-CRM bridge. Accepts `{maid, location_tag, timestamp}` from geofence hardware. Step 1: Resolves identity via configurable broker API (FullContact or similar) using `IDENTITY_API_URL` and `IDENTITY_API_KEY` env vars. Step 2: Creates CRM contact tagged `Crash_Connect_Lead` + `Sentinel_Geofence`. Step 3: Logs sentinel incident. Step 4: Optionally pushes to LeadConnector V2 API using `APEX_CRM_URL` and `APEX_API_KEY` env vars (with `Version: 2021-07-28` header). Returns 200 immediately; processing runs async. Falls back to storing raw MAID if no broker configured.
