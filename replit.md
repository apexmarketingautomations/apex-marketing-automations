...
- **Files**: `server/routes/tracking.ts` (capture + ingest + admin + identify + analytics routes), `server/services/trackingIdentity.ts`, `server/services/trackingSnapshots.ts`. Registered in `server/routes.ts` via `registerTrackingRoutes(app)`.