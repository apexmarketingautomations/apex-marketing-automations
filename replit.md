...
- **AI & Messaging**: Features an AI Gateway for centralized routing (OpenAI primary, Gemini fallback with circuit breaker), Officer Layla AI Persona for DMs, a Unified Inbox for multi-channel communication, and a Comment Auto-Reply Bot utilizing pgvector and OpenAI embeddings. Apex Operator acts as a cognitive intelligence layer for natural language task interpretation and multi-step execution. The DM context assembler (`server/dmContextAssembler.ts`) uses a full persona override path when `customAiPrompt` > 200 chars — this path strips generic CRM blocks (brandVoice, serviceGuardrail, primaryGoal, anti-robot, escalation) to prevent persona contamination.
