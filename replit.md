...
- **Autonomous Task Agent**: A background worker that scans sub-accounts, detects issues and opportunities, and executes corrective actions using the Operator's tool registry. It includes auto-task definitions, tool execution, and safety mechanisms with per-account configurations.
