...
- **UI/UX**: Mobile-responsive design, guided 5-step onboarding wizard, and an Apex Intelligence premium panel with advanced analytics and AI interaction features, including a dynamic Operator tab and site-assistant. The panel is draggable (entire header bar + toggle button), resizable (bottom-right corner handle), and position-agnostic (supports `inline`, `bottom-left`, `bottom-right`, `top-left`, `top-right`). Props: `position`, `defaultOpen`, `accountId`, `showToggle`, `panelWidth`, `panelHeight`.
