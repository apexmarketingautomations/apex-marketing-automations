...
- **Liquid Website Protocol** (`/liquid`): Full "shape-shifting" AI landing page engine. Template variable engine resolves `{{contact.first_name | default: 'Welcome'}}` and `{{url_param.heading}}` syntax. URL parameter injection from ad URLs (`?heading=`, `?subheading=`, `?cta=`, `?offer=`, `?badge=`, `?vibe=`). Sticky Contact via localStorage + CRM lookup (`POST /api/liquid/contact-lookup`, public). Form submissions wired to `/api/form-submit` creating CRM contacts with "Liquid Site Lead" tag. Settings panel with domain setup instructions, template variable reference, ad URL builder, and testing checklist. AI generates 5 sections (HERO, FEATURES, TESTIMONIALS, BOOKING, CTA) personalized by device, referrer, time-of-day, contact name, and ad headline.
